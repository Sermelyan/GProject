...
public:
    Data_out();
    Data_out(const std::vector<std::pair<double, double >> &points, const int id, const int time);
    ~Data_out();
...
//    Worker(sqlite3 *conect, GQueue &In,GQueue &In);
    ...
    void GetDotsFromDB(std::vector<std::pair<double, double>> &points, Data_in value);
    void GetRibsFromAPI(std::vector<std::pair<double, double>> &points, std::vector<std::vector<double >> &ribs);
    void GetRoute(std::vector<std::pair<double, double>> &points, std::vector<std::vector<int>> &ribs, std::list<std::pair<double, double>> res);
    void Check();
